...
class NeuralCore():
    ...
    def __init__(self, device, path_folder, epoch, model_case, dataset_setting, univar_count, lat_dim, instaces_size, input_shape, seed=0, run_mode="all"):
        ...
        if run_mode=="fast":   
            ...
            self.ea_do__training_data = False
            self.ea_do__testing_data = False
            self.ea_do__noised_data = False
            self.ea_do__reduced_noised_data = False
            self.ea_do__copula_data = False
        ...
        if self.model_case=="GAN_linear_pretrained_16_PEMS_ds":
        ...
        # METR-LA
            ...
            if not os.path.exists(self.path_folder_ae):
                os.makedirs(self.path_folder_ae)
            self.loss_obj['AE'] = LossFunction({"JENSEN_SHANNON_DIVERGENCE_LOSS":1, "MEDIAN_LOSS_batch":0.005, "VARIANCE_LOSS":0.5, "SPEARMAN_CORRELATION_LOSS":1}, univar_count=self.univar_count, latent_dim=self.lat_dim, device=self.device)
            self.path_folder_gan = Path(self.path_folder,'GAN')
            if not os.path.exists(self.path_folder_gan):
                os.makedirs(self.path_folder_gan)
            self.loss_obj['GAN'] = LossFunction(dict(), univar_count=self.univar_count, latent_dim=self.lat_dim, device=self.device) 
        ...
        # CHENGDU        
            ...
            dataloader = DataLoader(mode="graph_roads", seed=self.seed_data, name_dataset="China_Chengdu_URB_zone0", time_slot="C", device=self.device, dataset_setting=self.dataset_setting, epoch = self.epoch, univar_count=self.univar_count, lat_dim=self.lat_dim, corrCoeff = self.corrCoeff, instaces_size=self.instaces_size, path_folder=self.path_folder)
            self.path_folder_ae = Path(self.path_folder,'AE')
            if not os.path.exists(self.path_folder_ae):
                os.makedirs(self.path_folder_ae)
            self.loss_obj['AE'] = LossFunction({"RMSE_LOSS":5e-3,"JENSEN_SHANNON_DIVERGENCE_LOSS":1, "MEDIAN_LOSS_batch":5e-1, "VARIANCE_LOSS":5e-1, "SPEARMAN_CORRELATION_LOSS":1e-3}, univar_count=self.univar_count, latent_dim=self.lat_dim, device=self.device)
            self.path_folder_gan = Path(self.path_folder,'GAN')
            if not os.path.exists(self.path_folder_gan):
                os.makedirs(self.path_folder_gan)
            self.loss_obj['GAN'] = LossFunction(dict(), univar_count=self.univar_count, latent_dim=self.lat_dim, device=self.device)
        ...
        self.data_splitted = dataloader.dataset_load(draw_plots=True, save_summary=True, loss=self.loss_obj, do__correlationCoeff=self.draw_correlationCoeff)
    ...
    def start_experiment(self, load_model=False):
        ...
        # CHENGDU
            ...
            self.optimization.set_modeltype(model_type="AE")
            search_space = [
                {"type":"Real","min":0.5,"max":1, "values_list":None, "name":"loss", "param":"COVARIANCE_LOSS"},
            ...
            model_ae_trained = self.training_model(data_dict=self.data_splitted, model_type="AE", model=self.model['AE'], loss_obj=self.loss_obj['AE'], epoch=self.epoch, graph_topology = self.graph_topology, 
                                                   optimization=True, optimizer_trial=self.optimization)
            self.predict_model(model=model_ae_trained, model_type="AE", data_dict=self.data_splitted, path_folder_pred=self.path_folder_ae,noise_samples=1000, input_shape="vector", draw_plot=self.draw_plot, draw_scenarios=self.draw_scenarios, draw_correlationCoeff=self.draw_correlationCoeff)   
            model_ae_decoder = model_ae_trained.getModel("decoder",train=True)            
    ...
    def training_model(self, data_dict, model_type, optimization=False, optimizer_trial=None,  model=None, loss_obj=None, pre_trained_decoder=False, epoch=None, model_flatten_in=True, load_model=False, graph_topology=False):
        if optimization:
            ...
            opt_objectivefunction = "test"
            self.optimization.optimization(n_trials=1, network_key = "AE", opt_objectivefunction =opt_objectivefunction)
        else:
    ...
    def predict_model(self, model, model_type, data_dict, input_shape, noise_samples=1000, draw_plot=True, draw_scenarios=True, draw_correlationCoeff=True, path_folder_pred=None):
        predMod = PerformePrediction(model=model, device=self.device,  model_type=model_type, univar_count=self.univar_count, latent_dim=self.lat_dim, data=data_dict, input_shape = input_shape, rangeData=self.rangeData, vc_mapping=self.vc_mapping, draw_plot=draw_plot, draw_scenarios=draw_scenarios, draw_correlationCoeff= draw_correlationCoeff, path_folder=path_folder_pred,  noise_samples=noise_samples)
        predMod.predict_model()
