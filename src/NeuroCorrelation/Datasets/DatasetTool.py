...
class DatasetTool:
    ...
    def load_fileJson(self, filepath, name_dataset, version_dataset):
        ...
        if config[name_dataset][version_dataset]["timeweath_path"] is not None:
            dataset_settings["timeweath_path"] = Path(config[name_dataset][version_dataset]["timeweath_path"])
        else:
            dataset_settings["timeweath_path"] = None
