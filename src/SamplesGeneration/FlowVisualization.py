...
class FlowVisualization():
    ...
    def __init__(self, simulation_name, sampled_dir, sample_id, load_data=True, sampled_geograph=None, edge_stats=None):
        ...
        if load_data:
            edge_stats_path = Path(self.sampled_dir,self.simulation_name+'.random.'+sample_id_str+'.edgestats.json')  
            with open(edge_stats_path) as json_file:
                self.edge_stats = json.load(json_file)
            
            sampled_geograph_path = Path(self.sampled_dir,self.simulation_name+'.random.'+sample_id_str+'.graph.graphml')
    ...
    def draw_sampledgraph(self, attr='travel_time',nbins=10):
        ...
        edge_col = self.edgecolors_attr(self.sampled_geograph, attr='travel_time', nbins=nbins)
    ...
    def edgecolors_attr(self, G, attr, nbins, cmap='viridis', start=0, stop=1, na_color='none'):
        edges_attributes = G.edges(keys=True, data=True)
        att_val_list = [edge[3][attr] for edge in list(edges_attributes)]
        attr_values = pd.DataFrame(att_val_list)  # Cretaes a dataframe ith the attribute of each node
