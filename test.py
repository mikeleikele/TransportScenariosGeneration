...
def neuroDist(num_case, folder, load_model=None):
    ...
    modelCase = ['autoencoder_6k_Chengdu','autoencoder_3_copula','autoencoder_3_defined','autoencoder_16_PEMS',"autoencoder_16_MetrLa","autoencoder_ALL_PEMS","autoencoder_ALL_MetrLa", "GAN_linear_16_PEMS", "GAN_linear_pretrained_vc_copula", "GAN_conv_vc_7_copula", "AE_conv_vc_copula"]
    print(f"|------------------------")
    print(f"| Modelcase   : {modelCase[int(num_case)]}")
    print(f"|------------------------")
    print(f" ")
    nc = NeuralCore(device,model_case=modelCase[int(num_case)], path_folder=folder)
    if load_model=="--load":
